...
from op_tiling_analyzer.models import BlockUnit, PlatformProfile, PromptFlashAttentionCase
from op_tiling_analyzer.utils import ceil_div, cpp_trunc_div, literal_parse
...
class PromptFlashAttentionV2Replayer:
    def __init__(self, repo_root: Path, source_root: Path | None = None) -> None:
        ...
        self.source_root = (source_root or (self.repo_root / "prompt_flash_attention")).resolve()
    ...
    def replay_case(self, case: PromptFlashAttentionCase, platform: PlatformProfile) -> dict[str, Any]:
        ...
        source_hash = hashlib.sha256(
            self.source_path.read_bytes() + self.header_path.read_bytes()
        ).hexdigest()[:12]
        return {
